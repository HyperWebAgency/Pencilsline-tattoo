...
/**
 * The deal places 12 cards, but the pool it draws from should be larger: the
 * provider shuffles before dealing, so a bigger pool means every photo can turn
 * up rather than the same first twelve every time. At exactly 12 the newest
 * photos were cut off by display_order and never appeared at all.
 */
const DECK_LIMIT = 30
